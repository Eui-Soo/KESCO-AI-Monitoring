...
class MonitorService:
            ...
            priority_sites = [
                {
                    ...
                    "reason": "High anomaly score" if site.get("latest_score", 0) >= 71 else "Score increased / warning zone",
                    "score": site.get("latest_score"),
                    "risk_level": site.get("status"),
                    "recommend_action": site.get("recommend_action") or "Check Bank/Rack/Module detail view",
                }
    ...
    async def get_recommendations(self, target_level: str = "module", site_id: Optional[str] = "SITE-0021", bank_id: Optional[str] = None, rack_id: Optional[str] = None, string_id: Optional[str] = None, module_id: Optional[str] = None, cell_no: Optional[int] = None) -> dict[str, Any]:
        ...
        return {"status": "success", "result": [{"recommendation_id": "REC-0001", "target_level": target_level, "target_id": target_id, "severity": "abnormal", "title": "Detailed inspection required", "message": "High score was detected from anomaly_score.", "reason": "Risk based on latest analysis result", "action": "Check Bank/Rack/String/Module/Cell detail and decide field inspection.", "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}, {"recommendation_id": "REC-0002", "target_level": target_level, "target_id": target_id, "severity": "warning", "title": "Watch analysis trend", "message": "Showing latest analysis results until trend API is connected.", "reason": "v11: dashboard/list/module detail DB bridge", "action": "Connect trend query by target_date in the next step.", "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}], "data_source": "local-db-aware"}
