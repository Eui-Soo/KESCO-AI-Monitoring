"""KESCO AI Monitoring Web API

내부망 AI 모니터링 웹 화면에서 사용할 1차 API 라우터다.
현재 버전은 화면 연동 구조를 고정하기 위한 mock 데이터를 반환한다.
추후 동일한 응답 구조를 유지한 상태에서 로컬 DB/anomaly_score/pipeline_run_log 기반으로 교체한다.
"""
...
def _status_counts() -> dict:
    return {
        "normal_site_count": sum(1 for site in SITES if site["status"] == "normal"),
        "warning_site_count": sum(1 for site in SITES if site["status"] == "warning"),
        "abnormal_site_count": sum(1 for site in SITES if site["status"] == "abnormal"),
        "unavailable_site_count": sum(1 for site in SITES if site["status"] == "unavailable"),
        "data_missing_site_count": sum(1 for site in SITES if site["status"] == "data_missing"),
    }


def _risk_ranking(limit: int = 10) -> list:
    ...
    return [
        {
            ...
            "region": site["region"],
            "install_area": site["install_area"],
            "manufacturer": site["manufacturer"],
            "latest_score": site["latest_score"],
            "status": site["status"],
            "status_text": site["status_text"],
            "last_analysis_time": site["last_analysis_time"],
            "risk_location": site["risk_location"],
        }
        for index, site in enumerate(ranked[:limit])
    ]


def _tree(site: dict) -> dict:
    return {
        "site_id": site["site_id"],
        "site_name": site["site_name"],
        "tree": [
            {
                "bank_id": "BANK-01",
                "bank_name": "Bank 01",
                "status": "abnormal" if site["status"] == "abnormal" else site["status"],
                "score": site["latest_score"] or 0,
                "racks": [
                    {
                        "rack_id": f"RACK-{rack_no:02d}",
                        "rack_name": f"Rack {rack_no:02d}",
                        "status": "abnormal" if rack_no == 3 and site["status"] == "abnormal" else "normal",
                        "score": 86.5 if rack_no == 3 and site["status"] == "abnormal" else 28.2 + rack_no,
                        "strings": [
                            {
                                "string_id": f"STRING-{string_no:02d}",
                                "string_name": f"String {string_no:02d}",
                                "status": "abnormal" if rack_no == 3 and string_no == 2 and site["status"] == "abnormal" else "normal",
                                "score": 79.4 if rack_no == 3 and string_no == 2 and site["status"] == "abnormal" else 25.0 + string_no,
                                "modules": [
                                    {
                                        "module_id": f"MODULE-{module_no:02d}",
                                        "module_name": f"Module {module_no:02d}",
                                        "status": "abnormal" if rack_no == 3 and string_no == 2 and module_no == 5 and site["status"] == "abnormal" else "normal",
                                        "score": 93.7 if rack_no == 3 and string_no == 2 and module_no == 5 and site["status"] == "abnormal" else 18.0 + module_no,
                                    }
                                    for module_no in range(1, 7)
                                ],
                            }
                            for string_no in range(1, 5)
                        ],
                    }
        ...
        cells.append(
            {
                ...
                "voltage": round(3.63 - (index * 0.006) + (score / 1000), 3),
                "temperature": round(31.5 + (score / 8), 1),
                "internal_resistance": round(1.05 + (score / 120), 2),
                "capacity": round(99.0 - (score / 30), 1),
                "soc": round(78.5 - (index % 5), 1),
                "trend_score": score + 4,
            }
        )
    return cells
    ...
    return [
        {
            ...
            "max_score": value,
            "avg_score": round(value * 0.68, 1),
            "status": "abnormal" if value >= 71 else "warning" if value >= 40 else "normal",
        }
        for index, value in enumerate(values[-days:])
    ]
...
async def get_level_detail(
    ...
    if level == "module":
        result = {
            **base,
            "module_name": "Module 05",
            "module_no": 5,
            "module_voltage": 74.1,
            "module_current": 38.2,
            "module_temperature": 33.6,
            "soc": 78.0,
            "soh": 96.2,
            "danger_cell_count": 1,
            "warning_cell_count": 3,
            "normal_cell_count": 16,
            "cells": _cells(),
        }
    elif level == "string":
        result = {
            **base,
            "string_name": "String 02",
            "string_no": 2,
            "module_count": 6,
            "danger_cell_count": 3,
            "string_voltage": 444.6,
            "string_current": 38.2,
            "temperature_avg": 32.4,
            "modules": [
                {"module_id": f"MODULE-{i:02d}", "module_name": f"Module {i:02d}", "module_no": i, "status": "abnormal" if i == 5 else "normal", "max_score": 93.7 if i == 5 else 20 + i, "avg_score": 54.2 if i == 5 else 18.5 + i, "voltage": 74.0 + i / 10, "current": 38.2, "temperature": 31.5 + i / 2, "danger_cell_count": 1 if i == 5 else 0, "worst_cell_no": 7 if i == 5 else None}
                for i in range(1, 7)
            ],
        }
    elif level == "rack":
        result = {
            **base,
            ...
            "rack_voltage": 1778.4,
            "rack_current": 38.2,
            "rack_temperature_avg": 31.8,
            "strings": [
                {"string_id": f"STRING-{i:02d}", "string_name": f"String {i:02d}", "string_no": i, "status": "abnormal" if i == 2 else "normal", "max_score": 79.4 if i == 2 else 28 + i, "avg_score": 46.2 if i == 2 else 21 + i, "module_count": 6, "danger_module_count": 1 if i == 2 else 0, "danger_cell_count": 3 if i == 2 else 0}
                for i in range(1, 5)
            ],
        }
    else:
        result = {
            **base,
            ...
            "abnormal_rack_count": 1,
            "offline_rack_count": 0,
            "racks": [
                {"rack_id": f"RACK-{i:02d}", "rack_name": f"Rack {i:02d}", "rack_no": i, "status": "abnormal" if i == 3 else "normal", "max_score": 86.5 if i == 3 else 31 + i, "avg_score": 52.7 if i == 3 else 22 + i, "string_count": 4, "module_count": 24, "danger_cell_count": 5 if i == 3 else 0}
                for i in range(1, 5)
            ],
        }
        ...
        "result": [
            {
                ...
                "message": "Cell 07에서 과열 및 전압 편차가 동시에 확인되었습니다.",
                "reason": "이상 점수 92/100, 전압 편차 +128mV, 온도 48.7℃",
                "action": "냉각 상태 점검 및 셀 단위 전압/저항 측정을 수행하세요.",
                "created_at": NOW_TEXT,
            },
            {
                ...
                "target_id": cell_no or module_id or string_id or rack_id or bank_id or site_id,
                "severity": "warning",
                "title": "정기 점검 주기 단축 권장",
                "message": "최근 7일간 Module 이상 점수가 상승 추세입니다.",
                "reason": "32 → 58 구간 상승 후 주의 구간 유지",
                "action": "다음 점검을 3일 이내로 설정하고 추이를 관찰하세요.",
                "created_at": NOW_TEXT,
            },
        ],
    }
